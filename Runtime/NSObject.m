...
{
	NSLog( @"<%s 0x%p> asked to forward:: %s", object_getClassName(self), self, sel_getName(selector) );
	return nil;
}
...
{
	PF_HELLO("")
	return [NSString stringWithFormat: @"<%s: 0x%p>", class_getName([self class]), self];
}

// this version returns the class name and address
+ (NSString *)description
{
	PF_HELLO("")
	return [NSString stringWithFormat: @"<%s: 0x%p>", class_getName(self), self];
}
